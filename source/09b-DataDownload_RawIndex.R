local_path <- 'D:\\DataAnalyticsPortal\\'
server_path <- '/srv/shiny-server/DataAnalyticsPortal/'
path = server_path

IndexDownload <- read.fst(paste0(path,'data/DT/IndexDownload.fst'))
load(paste0(path,'data/Updated_DateTime_ConsumptionIndexDownload.RData'))
load(paste0(path,'data/Week.date.RData'))

output$Indexdatadownload_customperiod <- renderUI({
  if(input$Indexdatadownload_period=='0'){
    dateRangeInput('dateRange',
                   label = h3('Date range'),
                   start = max(IndexDownload$date)-7, end = max(IndexDownload$date),separator = " - ", format = "dd/mm/yyyy",
                   min = begin_date, max=today()
    )
  }else{NULL}
})

start_Indexdatadownload <- reactive({
  this_day <- Sys.Date()
  temp <- switch(input$Indexdatadownload_period,
                 "0" = as.Date(as.character(input$dateRange[1]),tz="Asia/Singapore"),
                 "1" = this_day,
                 "2" = this_day-1,
                 "3" = as.Date(Week.date$beg[match(IndexDownload$week[match(this_day-1,IndexDownload$date)[1]],Week.date$week)]),
                 "4" = as.Date(Week.date$beg[match(IndexDownload$week[match(this_day-1,IndexDownload$date)[1]],Week.date$week)-1]),
                 "5" = this_day-7,
                 "6" = floor_date(this_day,unit='month'),
                 "7" = floor_date(this_day,unit='month')-months(1), 
                 "8" = this_day-30
                )
  as.character(temp)
})

end_Indexdatadownload <- reactive({
  this_day <- Sys.Date()
  temp <- switch(input$Indexdatadownload_period,
                 "0"=as.Date(input$dateRange[2]+1,tz='Asia/Singapore'),
                 "1"=this_day+1,
                 "2"=this_day,
                 "3"=this_day+1,
                 "4"=as.Date(Week.date$beg[match(IndexDownload$week[match(this_day-1,IndexDownload$date)[1]],Week.date$week)]),
                 "5"=this_day+1,
                 "6"=this_day+1,
                 "7"=floor_date(this_day,unit='month'),
                 "8"=this_day+1
                )
  as.character(temp)
})

filtered_index <- reactive({
  IndexDownload %>% dplyr::filter(date >= ymd(start_Indexdatadownload()) & date < ymd(end_Indexdatadownload())) %>%
                          dplyr::arrange(date) %>% select(-week)
})

output$Index_table <-  DT::renderDataTable(tail(filtered_index(),100),rownames=FALSE,
                                       options = list(scrollX=FALSE,pageLength=10,bFilter=0,searching=FALSE))
# global search box off (bFilter=0)

output$Index_table_info <- renderUI({
  fluidRow(
    column(12,HTML('<table> 
                   <tr>
                   <td> <align="justify">
                   This section displays the raw index (meter readings) before any computation. Index (meter readings) are in litres. <br>
                   As a preview, the above table displays only the last 100 rows of filtered data.<br>
                   Also, take note in the Custom selection, only the last 3 months of data is available.
                   However, Suez recommends to download the data on a monthly basis, or 30 days individually.<br> 
                   In the case of 1200 households, each file would contain less than 1 million rows which is compatible with an Excel spreadsheet.
                   For a file which contains more than 1 million rows, it is advisable to open it with Notepad++.<br> 
                   Take note that meter_type of MAIN refers to Master meters.
                
                   </td>
                   </tr>
                   </table>'))
  )
})

# download the filtered data, https://yihui.shinyapps.io/DT-info/
# output$downloadConsumptionData <- downloadHandler('consumption-filtered.csv', 
#                                                    content = function(file) {
#                                                    s = input$Consumption_table_rows_all
#                                   write.csv(filtered_consumption()[s, , drop = FALSE], file)
# })

output$downloadIndexData <- downloadHandler(
  filename = function() { paste('Index', '.csv', sep='') },
  content = function(file) {
    write.csv(filtered_index(), file, row.names = FALSE)
  }
)

output$lastUpdated_downloadIndex <- renderText(Updated_DateTime_ConsumptionIndexDownload)