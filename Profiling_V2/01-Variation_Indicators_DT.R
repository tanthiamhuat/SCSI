...
Punggol_SUB <- consumption_last12months_servicepoint[!(room_type %in% c("NIL","HDBCD","OTHER","Nil")) & !(is.na(room_type)) & 
                                                       meter_type =="SUB" & site == "Punggol" & !is.na(adjusted_consumption)]

X <- Punggol_SUB
X$ID <- as.character(X$service_point_sn);
X$Consumption <- X$adjusted_consumption
X$Date <- date(X$date_consumption)
X$D <- day(X$date_consumption)
X$H <- hour(X$date_consumption)
X$M <- month(X$date_consumption)
X$Y <- year(X$date_consumption)
X$wd <- weekdays(X$date_consumption)

## remove data belong to today if there are no full set
if(X$H[which.max(X$date_consumption)]!=23){
  X <- X %>% dplyr::filter(Date < max(X$Date))
  }
...
##################################### Hour #################################################
X$hour <- paste0("H",sprintf('%02d',hour(X$date_consumption)))
