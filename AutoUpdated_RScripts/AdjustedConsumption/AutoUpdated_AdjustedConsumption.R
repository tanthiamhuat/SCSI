...
# sql_update <- paste("UPDATE consumption SET adjusted_consumption = '",Correct.adjusted_consumption$adjusted_consumption,"'
#                      WHERE interpolated_consumption = '",Correct.adjusted_consumption$interpolated_consumption,"' and
#                      index_value = '",Correct.adjusted_consumption$index_value,"' and
#                      id_service_point = '",Correct.adjusted_consumption$id_service_point,"' ")

# sql_update <- paste("UPDATE consumption SET adjusted_consumption = '",Correct.adjusted_consumption$adjusted_consumption,"'
#                      WHERE interpolated_consumption = '",Correct.adjusted_consumption$interpolated_consumption,"' and
#                      date_consumption = '",Correct.adjusted_consumption$date_consumption,"' and
#                      id_service_point = '",Correct.adjusted_consumption$id_service_point,"' ")

## above sql_update of a column of more than 3 million rows has no effect.

# sql_update_test <- paste("UPDATE consumption SET adjusted_consumption = 412345
#                           WHERE interpolated_consumption = 4 and
#                           index_value = 4671257 and
#                           id_service_point = 591 ")

# for (i in 1:length(sql_update))
# {
#   dbSendQuery(mydb, sql_update[i])
# }

# length(sql_update)
# dbSendQuery(mydb, sql_update)

write.csv(Correct.adjusted_consumption,"AdjustedConsumption.csv")

from ="/srv/shiny-server/DataAnalyticsPortal/source/AdjustedConsumption.csv"
to="/opt/kettle-files/16"
file.copy(from, to, overwrite = TRUE, recursive = F, copy.mode = TRUE)
