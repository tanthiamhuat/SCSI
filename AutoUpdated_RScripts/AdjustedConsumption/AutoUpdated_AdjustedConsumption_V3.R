# Adjusted Consumption to be modified to cater for Status="Interpolated value on missing index" 
...
consumption <- as.data.frame(tbl(con,"consumption")) %>% filter(date(date_consumption)>="2017-04-06")
...
Z <- consumption_servicepoint %>% filter(id_service_point==293)
...
Correct.adjusted_consumption3 <- Correct.adjusted_consumption %>% filter(id_service_point %in% c("601"))
