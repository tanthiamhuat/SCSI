...
SFTP_directory <- "sftp://SINGAPORE@sftp.sitr.suezsmartsolutions.com:8122/SINGAPORE/Data/Output/index_processed/"
...
## get digits >= 171 and !(479,808) ## get this from SFTP Index files
Downloaded_IndexFiles_digits_Integer <- as.data.frame(as.numeric(Downloaded_IndexFiles_digits))
colnames(Downloaded_IndexFiles_digits_Integer) <- "digits"  
Downloaded_IndexFiles_digits_Integer <- Downloaded_IndexFiles_digits_Integer %>%
                                        filter(digits>= 171 & !(digits %in% seq(479,808)))
...
library(threadr)
credentials <- "SINGAPORE:S1n9ap0r3PF337£"
SFTP_directory <- "sftp://SINGAPORE@sftp.sitr.suezsmartsolutions.com:8122/SINGAPORE/Data/Output/index_processed/"
a=file_information(list_files_ftp(SFTP_directory,credentials))
