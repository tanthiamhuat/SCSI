...
PunggolYuhuaConsumption <- inner_join(PunggolYuhua_SUB,family_servicepoint,by=c("service_point_sn","block","floor","unit","room_type","site","meter_type")) 
...
DailyPunggolYuhuaConsumption_past3months <- PunggolYuhuaConsumption %>%
  dplyr::filter(date(date_consumption) >= move_in_date) %>%  # take into consideration of move-in-date
  dplyr::filter(as.numeric(difftime(today,move_in_date,units="days"))>90) %>%
  dplyr::mutate(Date=date(date_consumption),yearmonth=substr(Date,1,7)) %>%
...
threshold=50
  Overconsumption <- inner_join(DailyPunggolYuhuaConsumption_last7days,DailyPunggolYuhuaConsumption_past3months,
...
family_servicepoint_overconsumption_DAP <- inner_join(family_servicepoint,overconsumption_DAP,by=c("service_point_sn","block")) %>%
  dplyr::select_("id","service_point_sn","meter_sn","block","overconsumption_date","online_status") %>%
  dplyr::rename(family_id=id) %>% 
  dplyr::arrange(desc(overconsumption_date))

Updated_DateTime_OverConsumption_Alarm <- paste("Last Updated on ",now(),"."," Next Update on ",now()+7*24*60*60,".",sep="")

save(family_servicepoint_overconsumption_DAP,Updated_DateTime_OverConsumption_Alarm,file="/srv/shiny-server/DataAnalyticsPortal/data/overconsumption.RData")
...
ans <- paste("AutoUpdated_OverConsumption_Alarm_DT successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
