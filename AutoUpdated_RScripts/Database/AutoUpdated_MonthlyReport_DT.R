...
PunggolYuhuaConsumption_SUB <- PunggolYuhua_SUB %>%
  dplyr::mutate(year=year(date_consumption),month=month(date_consumption),day=day(date_consumption)) %>%                  
  select(service_point_sn,block,room_type,floor,adjusted_consumption,date_consumption,year,month,day) %>%
...
PunggolYuhuaConsumption_data <- PunggolYuhuaConsumption_SUB %>%
  dplyr::filter(month(date_consumption) %in% c(lastmonth,last2month) & year(date_consumption) %in% c(lastyear,thisyear)) %>%  
  dplyr::mutate(wd=weekdays(date(date_consumption)),hour=hour(date_consumption)) %>%
  select(service_point_sn,adjusted_consumption,date_consumption,wd,hour,day,month,year) 

MonthlyConsumption <- PunggolYuhuaConsumption_data %>% 
...
DailyConsumption <- PunggolYuhuaConsumption_data %>% 
                    dplyr::mutate(Year=year(date_consumption),Date=date(date_consumption),wd=weekdays(date(date_consumption))) %>%
...
HourlyConsumption <- PunggolYuhuaConsumption_data %>% 
...
ans <- paste("AutoUpdated_MonthlyReport_DT successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
