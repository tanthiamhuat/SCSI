...
weeknumber <- as.numeric(strftime(last7days[7],format="%W")) 
if (weeknumber <10) {weeknumber <- paste(0,weeknumber,sep="")}
WeekNumber=paste(year(last7days[7]),"_",weeknumber,sep="") 

weekly_occupancy <- as.data.frame(tbl(con,"weekly_occupancy")) %>% dplyr::filter(week_number==WeekNumber)
monthly_occupancy1 <- monthly_occupancy %>% dplyr::mutate(month=month(lastupdated)) %>% filter(month==1)
...
overconsumption_alarm_weeklyoccupancy <- inner_join(overconsumption_alarm,weekly_occupancy,by="service_point_sn") %>%
                                         dplyr::filter(occupancy_rate <50) %>%
                                         dplyr::select_("service_point_sn")
overconsumption_alarm_monthlyoccupancy <- inner_join(overconsumption_alarm,monthly_occupancy1,by="service_point_sn") %>%
  dplyr::filter(occupancy_rate <50 & occupancy_rate!=0) %>%
  dplyr::select_("service_point_sn")

          
leak_alarm_past7days <- leak_alarm %>% filter(meter_type=="SUB" & start_date %in% last7days) %>% 
                        dplyr::select_("service_point_sn")
overconsumption_exclude <- rbind(overconsumption_alarm_weeklyoccupancy,leak_alarm_past7days)

overconsumption_alarm <- anti_join(overconsumption_alarm,overconsumption_exclude,by="service_point_sn")
