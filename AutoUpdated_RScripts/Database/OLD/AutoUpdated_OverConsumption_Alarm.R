...
Punggol_All <- consumption_last6months_servicepoint[site=="Punggol"]
...
family_servicepoint_overconsumption_DAP <- inner_join(family_servicepoint,overconsumption_DAP,by=c("service_point_sn","block")) %>%
  dplyr::select_("id.x","service_point_sn","meter_sn","block","overconsumption_date","online_status") %>%
  dplyr::rename(family_id=id.x) %>% 
  dplyr::arrange(desc(overconsumption_date))
...
ans <- paste("AutoUpdated_OverConsumption_Alarm successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
