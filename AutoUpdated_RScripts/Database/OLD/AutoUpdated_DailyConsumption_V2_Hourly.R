...
## Check for yesterday consumption to be full.
YesterdayConsumption <- inner_join(PunggolConsumption_SUB,family_servicepoint,by="service_point_sn") %>%
  dplyr::mutate(date=date(date_consumption)) %>%
  dplyr::filter(service_point_sn !="3101127564" && date==last7days[1]) # remove Child-Care

group_by(service_point_sn,month,day,wd) %>%
  dplyr::summarise(YesterdayConsumption=sum(adjusted_consumption)) %>%
...
DailyConsumption_past3months_weekdays <- DailyConsumption %>%
  ...
  dplyr::filter(month %in% last3months) %>%
