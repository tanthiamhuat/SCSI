...
for (i in 10:1){  # date_consumption from 18 Dec 2016 to 27 Dec 2016
last7days <- c(Sys.Date()-i-c(1:7))
...
DailyConsumption_past3months_weekdays <- DailyConsumption %>%
  ...
  dplyr::filter(month %in% (month(Sys.Date())-c(1,2,3))) %>%
  dplyr::summarise(Avg_3mth_wd=round(mean(DailyConsumption))) 
threshold=50
daily_consumption[[i]] <- inner_join(DailyPunggolConsumption_last7days,DailyConsumption_past3months_weekdays,
...
daily_consumption <- daily_consumption_leak_alarm %>%
  ...
  dplyr::arrange(date_consumption)
...
ans <- paste("AutoUpdated_DailyConsumption_LOOP successfully completed in",round(time_taken[3],2),"seconds.")
print(ans)