...
months_numeric <- c(1,2,3,4,5,6,7,8)

monthly_report1 <- list()

i=1 # for Jan and Feb 2017
...
for (i in 1:length(months))
{
  PunggolConsumption_SUB <- Punggol_All %>%
    ...
    dplyr::mutate(day=D,month=M,year=Y) %>%                  
    select(service_point_sn,block,room_type,floor,adjusted_consumption,adjusted_date,day,month,year) %>%
    ...
    monthlyoccupancy <- as.data.frame(tbl(con,"monthly_occupancy")) %>% 
                        ...
                        dplyr::filter(service_point_sn !="3101127564" & # exclude child-care
                                     (Year==thisyear & Month %in% c(months_numeric[i+1],months_numeric[i])))
    ...
    PunggolConsumption_data <- PunggolConsumption_SUB %>%
         dplyr::filter(month(adjusted_date) %in% c(months_numeric[i+1],months_numeric[i]) & year(adjusted_date)==thisyear) %>%
    ...
    monthly_report1[[i]] <- as.data.frame(spread(average_dailyconsumption_occupiedDays,month,average_dailyconsumption))
}

monthly_report1 <- as.data.frame(rbindlist(monthly_report1, fill=TRUE))
colnames(monthly_report1)[2] <- "last_2month_average"
colnames(monthly_report1)[3] <- "last_month_average"
monthly_report1$last_month <- rep(month.abb[months_numeric[i+1]],nrow(monthly_report1))
monthly_report1$last_2month <- rep(month.abb[months_numeric[i]],nrow(monthly_report1))
