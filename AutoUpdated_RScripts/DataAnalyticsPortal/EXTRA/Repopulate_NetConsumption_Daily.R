...
## every Monday
#days <- seq(as.Date("2016-08-25"),as.Date("2017-07-27"),by=1)
days <- seq(as.Date("2017-07-01"),as.Date("2017-07-31"),by=1)
DailyNetConsumption <- list()
DailyNetConsumptionCSV <- list()
for (k in 1:length(days))
{
  consumption_MAIN <- PunggolConsumption_MAIN %>%
                      dplyr::filter(date(date_consumption) >= days[k])
  ...
  for(j in 1:length(blocks)) {
    ...
    DailyNetConsumption[[k]] <- cbind(Date=as.data.frame(rep(days[k],5)),
    ...
    DailyNetConsumptionCSV[[k]] <- cbind(Date=as.data.frame(rep(days[k],5)),
                                          Block=as.data.frame(all_blocks),
                                          IndirectMainMeters=as.data.frame(unlist(IndirectMainMeters)),
...
DailyNetConsumption <- as.data.frame(rbindlist(DailyNetConsumption))
DailyNetConsumptionCSV <- as.data.frame(rbindlist(DailyNetConsumptionCSV))
colnames(DailyNetConsumption) <- c("Date","Block","IndirectMainMeters","IndirectNetConsumption",
                                    "DirectMainMeters","DirectNetConsumption","TotalMainMeters","TotalNetConsumption",
                                    "TotalNetConsumptionRate")
colnames(DailyNetConsumptionCSV) <- c("Date","Block","IndirectMainMeters","IndirectSubMeters","IndirectNetConsumption",
