...
PG_flow <- read.csv("/srv/shiny-server/DataAnalyticsPortal/data/PG_flow.csv")
## above from /srv/shiny-server/DataAnalyticsPortal/AutoUpdated_RScripts/Consumption/Punggol_Index.R
...
servicepoint_meter_PG <- inner_join(servicepoint_PG,meter,by=c("service_point_sn"="id_real_estate","meter_type"))

PG_flow_servicepoint_meter <- inner_join(servicepoint_meter_PG,PG_flow,by=c("id.x"="id_service_point")) 

leaks_open <- PG_flow_servicepoint_meter %>% 
...
leaks_close <- PG_flow_servicepoint_meter %>% 
  dplyr::filter(MinCons_60==0) %>%
  group_by(service_point_sn) %>%
  select(service_point_sn,meter_sn,MinCons_60,date) 
...
write.csv(df_final,"/srv/shiny-server/DataAnalyticsPortal/data/Leak_Punggol.csv",row.names=FALSE)

PG_Open_Severity <- df_final %>% dplyr::filter(status=="Open")
PG_Open_Severity$duration <- as.integer(difftime(today(),PG_Open_Severity$start_date,units="days"))
PG_Open_Severity <- PG_Open_Severity %>%
...
PG_Open_Severity$service_point_sn <- as.character(PG_Open_Severity$service_point_sn)
PG_Open_Severity_Block <- inner_join(PG_Open_Severity,servicepoint,by="service_point_sn") %>%
  dplyr::select_("service_point_sn","meter_sn","MinCons_60","start_date","end_date","status","duration","severity","block","floor","unit")

write.csv(PG_Open_Severity_Block,file="/srv/shiny-server/DataAnalyticsPortal/data/PG_Leak_Blocks.csv") 
