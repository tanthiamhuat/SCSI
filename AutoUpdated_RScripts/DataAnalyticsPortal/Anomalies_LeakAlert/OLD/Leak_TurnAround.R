...
family_servicepoint_offline <- family_servicepoint %>% dplyr::filter(online_status=="INACTIVE" & site=="Punggol" & !room_type.x %in% c("MAIN","BYPASS","HDBCD"))

turnAround_P2_Online <- Punggol_Leak %>%
   dplyr::filter(service_point_sn %in% family_servicepoint_online$service_point_sn & status=="Close" & end_date >="2016-04-20" & end_date <="2017-06-10") %>%
...
turnAround_P2 <- Punggol_Leak %>%
  dplyr::filter((service_point_sn %in% family_servicepoint_online$service_point_sn | service_point_sn %in% family_servicepoint_offline$service_point_sn) &
                  status=="Close" & end_date >="2016-04-20" & end_date <="2017-06-10") %>%
  dplyr::summarise(average_duration = mean(duration))
