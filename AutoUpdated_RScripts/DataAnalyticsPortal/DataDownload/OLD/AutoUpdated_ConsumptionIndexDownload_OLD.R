...
consumption_download <- consumption %>% filter(date(date_consumption)>=begin_date) %>% collect()
index <- tbl(con,"index")
index_download <- index %>% filter(date(current_index_date)>=begin_date) %>% collect()
