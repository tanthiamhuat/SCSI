...
for (i in 1:length(blocks)){
  assign(paste("flow_servicepoint_meter_",blocks[i],sep=""), flow_servicepoint_meter_Punggol_function(blocks[i]))
}
...
# new_leak <- anti_join(LeakSeverity[,c(2:(ncol(LeakSeverity)-2),ncol(LeakSeverity))],
#                       leak_alarm[,c(2:(ncol(LeakSeverity)-2),ncol(LeakSeverity))]) 
...
sql_update2 <- paste("UPDATE leak_alarm SET duration = '",leak_status_changes$duration,"',
...
LeakSeverityPunggol <- LeakSeverity %>% filter(site!="Tuas") %>% arrange(desc(status)) 
LeakSeverityPunggol["id"] <- NULL

colnames(LeakSeverityPunggol)[7] <- "duration (days)"
colnames(LeakSeverityPunggol)[8] <- "min5flow_ave (litres/hour)"

LeakSeverityPunggol[(ncol(LeakSeverityPunggol)-1):ncol(LeakSeverityPunggol)] <- NULL
...
family_servicepoint_LeakSeverityPunggol_Comments <- inner_join(family_servicepoint,LeakSeverityPunggol,by=c("service_point_sn","meter_type","site")) %>%
                                            dplyr::select_("id","service_point_sn","meter_sn","meter_type","site","start_date","end_date",
                                                           "`duration (days)`","`min5flow_ave (litres/hour)`","severity","status.y") %>%
                                            dplyr::rename(family_id=id,status=status.y) %>% arrange(desc(status)) %>%
                                            dplyr::mutate(Comments="TO BE EDITED HERE")

load("/srv/shiny-server/DataAnalyticsPortal/data/LeakSeverity_Punggol.RData")
## update duration, min5flow_ave, severity for status='Open'
LeakSeverityPunggol_Open <- LeakSeverityPunggol %>% 
                            dplyr::filter(status=="Open")

sql_update <- paste0("UPDATE leak_alarm_dap SET duration = '",LeakSeverityPunggol_Open$`duration (days)`,"', 
                                                min5flow_ave = '",LeakSeverityPunggol_Open$`min5flow_ave (litres/hour)`,"',
                                                severity = '",LeakSeverityPunggol_Open$severity,"'
                      WHERE service_point_sn = '",LeakSeverityPunggol_Open$service_point_sn,"' AND 
                      start_date = '",LeakSeverityPunggol_Open$start_date,"'")
sapply(sql_update, function(x){dbSendQuery(mydb, x)})

## update rows which the status changes from Open to Close
# leak_changes_Close <- LeakSeverityPunggol %>% dplyr::filter(!is.na(end_date)) # from Open to Close
# leak_status_changes <- LeakSeverityPunggol %>% 
#   dplyr::filter(service_point_sn %in% leak_changes_Close$service_point_sn) %>%
#   dplyr::group_by(service_point_sn) %>%
#   dplyr::filter(id==max(id))
# 
...
# 
# 
# leak_alarm_DAP_new <- family_servicepoint_LeakSeverityPunggol_Comments %>% dplyr::filter(duration=="")
#dbWriteTable(mydb, "leak_alarm_DAP", leak_alarm_DAP_new, append=TRUE, row.names=F, overwrite=FALSE)
...
# save(family_servicepoint_LeakSeverity_Punggol, file="/srv/shiny-server/DataAnalyticsPortal/data/LeakSeverity.RData")
# write.csv(family_servicepoint_LeakSeverity_Punggol,"/srv/shiny-server/DataAnalyticsPortal/data/LeakSeverity_Punggol.csv",row.names=FALSE)
save(family_servicepoint_LeakSeverity_Punggol_Comments_new, file="/srv/shiny-server/DataAnalyticsPortal/data/LeakSeverity_Punggol.RData")
write.csv(family_servicepoint_LeakSeverity_Punggol_Comments,"/srv/shiny-server/DataAnalyticsPortal/data/LeakSeverity_Punggol.csv",row.names=FALSE)
