...
LeakSeverity <- LeakSeverity %>% dplyr::mutate(duration=ifelse(status=="Open",duration+1,duration))
...
## online_status = INACTIVE if start_date or end_date is not within the FirstLoginDate and Move_out_Date
## load in customer date when they first login with to the app
customer_firstlogin <- as.data.frame(tbl(con_amrcms,"customer_points")) %>%
  dplyr::filter(reward_category=="daily_login") %>%
  dplyr::mutate(Date=date(created_date)) %>%
  dplyr::group_by(customer_id) %>% 
  dplyr::filter(Date==min(Date)) %>%
  dplyr::select_("customer_id","Date") %>%
  dplyr::rename("FirstLoginDate"="Date")

customer_account_activation <- as.data.frame(tbl(con_amrcms,"customer_points")) %>%
  dplyr::filter(reward_category=="account_activation") %>%
  dplyr::mutate(Date=date(created_date)) %>%
  dplyr::group_by(customer_id) %>% 
  dplyr::filter(Date==min(Date)) %>%
  dplyr::select_("customer_id","Date") %>%
  dplyr::rename("AccountActivatedDate"="Date")

customer <- as.data.frame(tbl(con_amrcms,"customer")) %>%
            dplyr::select_("id","id_family")

customer_first_login <- inner_join(customer_firstlogin,customer,by=c("customer_id"="id"))
customer_first_login_family <- inner_join(customer_first_login,family,by=c("id_family"="id")) %>% 
                               dplyr::select_("id_service_point","FirstLoginDate","move_out_date")
customer_first_login_family[1] <- NULL
customer_first_login_family <- customer_first_login_family %>% 
                               dplyr::group_by(id_service_point) %>%
                               dplyr::filter(FirstLoginDate==min(FirstLoginDate))

customer_first_login_family_servicepoint <- inner_join(customer_first_login_family,servicepoint,by=c("id_service_point"="id")) %>%
                                            dplyr::filter(service_point_sn %in% leak_alarm_Punggol_new$service_point_sn) %>%
                                            dplyr::select_("id_service_point","FirstLoginDate","service_point_sn","move_out_date") %>% unique()

leak_alarm_Punggol_new_firstlogin <- left_join(leak_alarm_Punggol_new,customer_first_login_family_servicepoint,by="service_point_sn") %>%
                                     dplyr::mutate(online_status=ifelse(is.na(FirstLoginDate),"INACTIVE", 
                                                                 ifelse((start_date < FirstLoginDate),"INACTIVE","ACTIVE"))) 

leak_alarm_Punggol_new <- leak_alarm_Punggol_new_firstlogin[,1:12]
...
leak_alarm_Punggol$service_point_sn <- as.character(leak_alarm_Punggol$service_point_sn)

tmp=full_join(leak_alarm_Punggol_new,leak_alarm_Punggol,by=c("family_id","service_point_sn","meter_type","site",
                                                             "start_date","end_date","duration","min5flow_ave","severity",
                                                             "status","online_status"))
tmp1=tmp[,c(1:11,ncol(tmp))]
colnames(tmp1)[6:12] <- c("end_date","duration","min5flow_ave","severity","status","online_status","Comments")
tmp1 <- unique(tmp1) %>% as.data.frame()
## for status=Open, the previous row remains, so we need to get the maximum of the duration for the same service_point_sn
tmp1_Open <- tmp1 %>% dplyr::group_by(service_point_sn) %>%
             dplyr::filter(status=="Open") %>%
             dplyr::filter(duration==max(duration)) %>%
             as.data.frame()
tmp1_Close <- tmp1 %>% dplyr::filter(status=="Close")
tmp1 <- rbind(tmp1_Open,tmp1_Close)
leak_alarm_Punggol <- tmp1

Updated_DateTime_LeakAlarm <- paste("Last Updated on ",now(),"."," Next Update on ",now()+24*60*60,".",sep="")

save(leak_alarm_Punggol,Updated_DateTime_LeakAlarm,file="/srv/shiny-server/DataAnalyticsPortal/data/leak_alarm_Punggol.RData")
write.csv(leak_alarm_Punggol,"/srv/shiny-server/DataAnalyticsPortal/data/leak_alarm_Punggol.csv",row.names=FALSE)

time_taken <- proc.time() - ptm
ans <- paste("AutoUpdated_LeakAlarm successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
