...
TotalBlockPunggolConsumption <-  Punggol_All %>%
  ...
  dplyr::summarise(OverallConsumption=round(sum(adjusted_consumption,na.rm = TRUE)/1000))

BlockAverageDailyConsumption_After10June2017 <- DailyLPCD_Block %>% dplyr::filter(date>"2017-06-10" & date <= (today()-1)) %>% 
  dplyr::group_by(block) %>%
  dplyr::summarise(AverageConsumption=mean(TotalDailyConsumption)) %>%
  dplyr::mutate(ConsumptionAfterGamificationStarted=round(AverageConsumption*daydifference_2/1000)) %>%
  dplyr::select_("ConsumptionAfterGamificationStarted") 
...
BlockWaterSavings_LeakAlarm <- cbind(BlockWaterSaved_LeakAlarm[,c(1,5)],TotalBlockPunggolConsumption[,2]) %>%
                               dplyr::mutate(Savings_w_LeakAlarm_Percent=round(Savings_w_LeakAlarm/OverallConsumption*100,2))
BlockWaterSavings_Gamification <- cbind(BlockWaterSaved_Gamification[,c(1,5)],BlockAverageDailyConsumption_After10June2017) %>%
                                  dplyr::mutate(Savings_w_Gamification_Percent=round(Savings_w_Gamification/ConsumptionAfterGamificationStarted*100,2))

BlockWaterSavingsValues <- cbind(BlockWaterSavings_LeakAlarm,BlockWaterSavings_Gamification)
BlockWaterSavingsValues[5] <- NULL
