...
## need to consider case where move_in_date is not updated.
# select * from family where move_in_date is NULL and room_type !='MAIN' and room_type!='BYPASS'
Punggol_MoveInDate_NotUpdated <- familyALL_servicepoint %>% 
  dplyr::filter(!room_type %in% c("MAIN","BYPASS") & site=="Punggol" & status !="VACANT") %>%
...
OverallQuantities = data.table(ItemDescription=c("Qty (Master+ByPass) Meters","Qty SUB Meters",
                                ...
                                Punggol=c(as.data.table(tally(Punggol_Main))+as.data.table(tally(Punggol_ByPass)),as.data.table(tally(Punggol_Sub)),
                                          ...
                                          length(Punggol_Real_Vacant)+nrow(Punggol_MoveInDate_NotUpdated),nrow(unexpected_consumption)),
...
PunggolTuas_DistanceReadingRate$site <- c(rep("Punggol",nrow(Punggol_DistanceReadingRate)),rep("Tuas",nrow(Tuas_DistanceReadingRate)))
