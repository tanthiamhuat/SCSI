...
consumption_family_servicepoint_meter_supply_MAINBYPASSSUB <- consumption_last6months_servicepoint[family_servicepoint_meter_supply_PG,nomatch=0]
...
PunggolWeeklyNetConsumption_xts <- xts(WeeklyNetConsumptionCSV_TotalNetConsumption_wide,order.by=as.Date(WeeklyNetConsumptionDate$LastDayofWeek))
...
PunggolWeeklyNetConsumptionRate_xts <- xts(WeeklyNetConsumptionCSV_TotalNetConsumptionRate_wide,order.by=as.Date(WeeklyNetConsumptionDate$LastDayofWeek))
...
PunggolWeeklyNetConsumption_NA_table <- WeeklyNetConsumption %>% 
...
## PunggolWeeklyNetConsumption_NA_table is for the display in the DAP in table form, need this "NA" so that it can be displayed in the the table.

PunggolWeeklyNetConsumptionCSV <- WeeklyNetConsumptionCSV %>% dplyr::arrange(desc(LastDayofWeek))

write.csv(PunggolWeeklyNetConsumptionCSV,"/srv/shiny-server/DataAnalyticsPortal/data/NetConsumption/PunggolWeeklyNetConsumption.csv",row.names=FALSE)

assign_NA <- WeeklyNetConsumptionCSV[which(is.na(WeeklyNetConsumptionCSV$TotalNetConsumptionRate)),1:2]
PunggolWeeklyNetConsumption <- WeeklyConsumption %>% filter(site=="Punggol") %>% dplyr::group_by(block,week,supply) %>%
                                      dplyr::mutate(NetConsumption=sum(WeeklyConsumption[meter_type!="SUB"])-sum(WeeklyConsumption[meter_type=="SUB"]))

PunggolWeeklyNetConsumption_NA_list <- list()
for (j in 1:nrow(assign_NA)){
PunggolWeeklyNetConsumption_NA_list[[j]] <- PunggolWeeklyNetConsumption %>%
...
PunggolWeeklyNetConsumption_NA <- rbindlist(PunggolWeeklyNetConsumption_NA_list)
PunggolWeeklyNetConsumption_NA <- PunggolWeeklyNetConsumption_NA %>% unique()
PunggolWeeklyNetConsumption_NA <- PunggolWeeklyNetConsumption_NA %>%
                                  dplyr::mutate(NetConsumption=ifelse(is.na(NetConsumption),"NA",NetConsumption))

PunggolWeeklyNetConsumption_nonNA_list <- list()
for (j in 1:nrow(assign_NA)){
  PunggolWeeklyNetConsumption_nonNA_list[[j]] <- PunggolWeeklyNetConsumption %>%
    dplyr::filter(block == assign_NA[j,1] & LastDayofWeek == assign_NA[j,2] & !is.na(NetConsumption))
}
PunggolWeeklyNetConsumption_nonNA <- rbindlist(PunggolWeeklyNetConsumption_nonNA_list)

## remove PunggolWeeklyNetConsumption_nonNA from PunggolWeeklyNetConsumption, and add
## PunggolWeeklyNetConsumption_NA
...
PunggolWeeklyNetConsumption_Diff <- setdiffDF(as.data.frame(PunggolWeeklyNetConsumption), PunggolWeeklyNetConsumption_nonNA)

PunggolWeeklyNetConsumptionDetails_NA <- rbind(PunggolWeeklyNetConsumption_Diff,PunggolWeeklyNetConsumption_NA)
# PunggolWeeklyNetConsumptionDetails_NA is used in the NetConsumptionDetails

Updated_DateTime_NetConsumption_Weekly_Punggol <- paste("Last Updated on ",now(),"."," Next Update on ",now()+7*24*60*60,".",sep="")

save(PunggolWeeklyNetConsumption_xts,PunggolWeeklyNetConsumptionRate_xts,
     PunggolWeeklyNetConsumptionDetails_NA,PunggolWeeklyNetConsumption_NA_table,
     Updated_DateTime_NetConsumption_Weekly_Punggol,
     file="/srv/shiny-server/DataAnalyticsPortal/data/NetConsumption/PunggolWeeklyNetConsumption.RData")

time_taken <- proc.time() - ptm
ans <- paste("AutoUpdated_NetConsumption_Weekly_Punggol successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
