...
servicepoint_available <- servicepoint[,.(id,site,block,service_point_sn)] %>%  
                          filter(service_point_sn !="3100507837M" & service_point_sn != "3100507837B")
...
RawConsumption <- read.fst("/srv/shiny-server/DataAnalyticsPortal/data/DT/RawConsumption.fst",as.data.table=T)

# set the ON clause as keys of the tables:
setkey(RawConsumption,ExternalMeteringPointReference)
setkey(servicepoint,service_point_sn)

RawConsumption_servicepoint <- RawConsumption[servicepoint, nomatch=0]
RawConsumption_servicepoint <- RawConsumption_servicepoint[,.(ExternalCustomerReference,MeterSerialNumber,ReadingDate,site)]
