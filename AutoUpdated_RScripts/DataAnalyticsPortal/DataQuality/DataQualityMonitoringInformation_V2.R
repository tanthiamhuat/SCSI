...
index_2018 <- read.fst("/srv/shiny-server/DataAnalyticsPortal/data/DT/RawIndex_2018.fst",as.data.table = TRUE)[date(ReadingDate) < today() &
                                                                                                               ExternalMeteringPointReference !="3100660792"]
...
index2018_servicepoint_meter <- inner_join(servicepoint_meter,index_2018,by=c("service_point_sn"="ExternalMeteringPointReference"))

index2018_data_hourly <- index2018_servicepoint_meter %>%
  select(meter_sn,ReadingDate,block,site) %>%
  mutate(Date=substr(ReadingDate,1,10),hour=hour(ReadingDate)) %>%
...
ans <- paste("DataQualityMonitoringInformation_V2 successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
