...
load(paste0(path,'data/DailyHourlyIndexRate_last5days.RData'))
...
Receivers <- data.frame(customer = rownames(Receivers_LongLat),
                        ...
                        ReadingRate = c("154350409","154350410","154350406","154350403","123456789","987654321","112233445"),
...
MapDisplayTable_PunggolYuhuaTuas_last5days <- rbind(MapDisplayTable_PunggolYuhua,MapDisplayTable_Tuas)
colnames(MapDisplayTable_PunggolYuhuaTuas_last5days)[1] <- "block"
...
save(PunggolYuhuaTuas,PunggolYuhuaTuas_TransmitterReceiver,
     PunggolYuhuaTuas_DistanceReadingRate,MapDisplayTable_PunggolYuhuaTuas_last5days,
     Updated_DateTime_MapDisplay,
     file=paste0(path,'data/MapDisplay_last5days.RData'))

time_taken <- proc.time() - ptm
ans <- paste("MapDisplay_last5days successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
