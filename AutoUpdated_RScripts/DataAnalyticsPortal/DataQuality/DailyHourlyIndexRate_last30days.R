...
last30days <- today()-30
today <- today()
index_last30days <- as.data.frame(tbl(con,"index") %>% dplyr::filter(date(current_index_date) >= last30days &
                                                              date(current_index_date) < today & id_service_point !=601))
...
save(f,x,y,DailyIndexReadingRate,HourlyIndexReadingRate,HourlyIndexReadingRate_Punggol,HourlyIndexReadingRate_Tuas,HourlyIndexReadingRate_Yuhua,
     ...
     file=paste0(path,'data/DailyHourlyIndexRate_last30days.RData'))

time_taken <- proc.time() - ptm
ans <- paste("DailyHourlyIndexRate_last30days successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
