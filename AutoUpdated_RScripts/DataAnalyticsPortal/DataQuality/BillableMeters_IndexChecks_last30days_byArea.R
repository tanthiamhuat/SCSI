...
last30days <- today()-30
today <- today()
index_last30days <- as.data.table(tbl(con,"index") %>% filter(date(current_index_date) >= last30days &
...
BillableMeters <- index_last30days %>% dplyr::filter(id_service_point !='601') %>%  # exclude AHL
                  ...
                  group_by(id_service_point,Date) %>%
...
missing_service_point_sn <- index_tmp %>% group_by(Date) %>%
...
save(BillableMeters,ZeroIndex_service_point_sn_Meter_Count,IncompleteIndex_service_point_sn_Meter_Count,Updated_DateTime_BillableMetersIndexChecks,
     file="/srv/shiny-server/DataAnalyticsPortal/data/BillableMeters_IndexChecks_last30days.RData")
