...
require(pacman)
pacman::p_load(dplyr,lubridate,RPostgreSQL,stringr,ISOweek)
...
PunggolConsumption_SUB$week <- gsub("-W","_",str_sub(date2ISOweek(date(PunggolConsumption_SUB$date_consumption)),end = -3)) # convert date to week
...
DailyPunggolConsumption_last3months <- PunggolConsumption_SUB %>% 
                                       dplyr::mutate(date=date(date_consumption),highest_day=weekdays(date_consumption)) %>%
...
DailyPunggolConsumption_lastmonth <- PunggolConsumption_SUB %>%
                                    dplyr::mutate(wd=weekdays(date_consumption), 
...
HourlyPunggolConsumption_last3months <- PunggolConsumption_SUB %>% 
                                        dplyr::mutate(time=strftime(date_consumption,format="%H:%M:%S"),H=as.numeric(substr(time,1,2))) %>%
