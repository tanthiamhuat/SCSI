########################################################
######## CHALLENGES
######## to be run once per day (and only once)
########################################################
...
family_to_do <- setdiff(family$id_service_point,S01_done$family_id)

if(length(family_to_do) > 0){
  max.flow <- as.data.frame(tbl(con,"flow") %>% filter(id_service_point %in% family_to_do))
  max.flow$family_id <- max.flow$id_service_point 
...
leaks$family_id <- family$id_service_point[match(leaks$service_point_sn,family$service_point_sn)] ###
...
## Case 2 : no new leaks
S04_ended  <- S04_running %>% filter(challenge_end_date == current_date-days(1))
if(NROW(S04_ended)>0){
  last3months.leak <- leaks %>% filter(start_date > current_date-months(3)-days(1))
...
overc$family_id <- overc$service_point_sn
new_overc <- overc %>% filter(overconsumption_date == (current_date - days(1)))
...
if(length(failed.family)>0){
  ...
  Update_S03_Fail <- data.frame(code = S04_failed$code,
                                id = S04_failed$id,
                                family_id = S04_failed$family_id,
                                results = 'FAILED',
                                updated_date = current_time)
...
# SUCCESS
S03_ended  <- S03_running %>% filter(challenge_end_date == current_date-days(1))
if(NROW(S03_ended)>0){
  last3months.overc <- overc %>% filter(overconsumption_date > current_date-months(3)-days(1))
