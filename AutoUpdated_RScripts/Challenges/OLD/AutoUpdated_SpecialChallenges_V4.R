...
if(sum(running_leaks$status=='Close')>0){
  ...
  ### UPDATE results
  sql_update<- paste0("UPDATE special_challenges SET results = '",Update_S02$results,"', created_date  = '",Update_S02$created_date,"' WHERE id = ",Update_S02$id," AND family_id = ",Update_S02$family_id," AND code = '",Update_S02$code,"';")
...
new_leaks <- leaks %>% filter(status=='Open' & start_date==current_date-3) ## check with Benjamin & duration==3) 
...
if(length(failed_S04)>0){
  ...
  Update_S04_Fail <- data.frame(code = S04_failed$code,
                                ...
                                results = 'FAILED',
                                created_date = current_time)
  sql_update<- paste0("UPDATE special_challenges SET results = '",Update_S04_Fail$results,"', created_date  = '",Update_S04_Fail$created_date,"' WHERE id = ",Update_S04_Fail$id," AND family_id = ",Update_S04_Fail$family_id," AND code = '",Update_S04_Fail$code,"';")
  sapply(sql_update, function(x){dbSendQuery(mydb, x)})
}
...
if(NROW(S04_ended)>0){
  ...
  if(length(success.family)>0){
    ...
    Update_S04_Success <- data.frame(code = S04_success$code,
                                     ...
                                     results = 'SUCCESS',
                                     created_date = current_time)
    sql_update<- paste0("UPDATE special_challenges SET results = '",Update_S04_Success$results,"', created_date  = '",Update_S04_Success$created_date,"' WHERE id = ",Update_S04_Success$id," AND family_id = ",Update_S04_Success$family_id," AND code = '",Update_S04_Success$code,"';")
    sapply(sql_update, function(x){dbSendQuery(mydb, x)})
  }
}
...
if(exists("Create_S02")){  ## some problem running this line
...
if(exists("Create_S04")){  ## some problem running this line
...
if(length(failed.family)>0){
  ...
  Update_S03_Fail <- data.frame(code = S03_failed$code,
                                ...
                                results = 'FAILED',
                                created_date = current_time)
  sql_update<- paste0("UPDATE special_challenges SET results = '",Update_S03_Fail$results,"', created_date  = '",Update_S03_Fail$created_date,"' WHERE id = ",Update_S03_Fail$id," AND family_id = ",Update_S03_Fail$family_id," AND code = '",Update_S03_Fail$code,"';")
  sapply(sql_update, function(x){dbSendQuery(mydb, x)})
...
if(NROW(S03_ended)>0){
  ...
  if(length(success.family)>0){
    ...
    Update_S03_Success <- data.frame(code = S03_success$code,
                                     ...
                                     results = 'SUCCESS',
                                     created_date = current_time)
    sql_update<- paste0("UPDATE special_challenges SET results = '",Update_S03_Success$results,"', created_date  = '",Update_S03_Success$created_date,"' WHERE id = ",Update_S03_Success$id," AND family_id = ",Update_S03_Success$family_id," AND code = '",Update_S03_Success$code,"';")
    sapply(sql_update, function(x){dbSendQuery(mydb, x)})
  }
}
