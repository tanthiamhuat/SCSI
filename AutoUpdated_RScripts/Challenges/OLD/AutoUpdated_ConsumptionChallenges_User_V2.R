...
#####
  ...
  running.chall <- all_challenges %>% filter(results=='AVAILABLE')
  ...
  get_challenge_results <- function(df){
    ...
    res.chall <- switch(substr(df$code,1,3),
                        ...
                        'C02' = results_family$highest_week_result,
                        'C03' = results_family$highest_period_result)
    df.return <- data.frame(family_id = df$family_id, code = df$code, results = ifelse(res.chall,'SUCCESS','FAILED'), created_date = current_time)
    return(df.return)
  }
  ...
  results_challenge$created_date <- paste0("'",results_challenge$created_date,"'")
  results_challenge$id <- nrow(all_challenges) + 1:(nrow(results_challenge))
  ## INSERT challenges in table 
  sql_insert<- paste0("INSERT INTO consumption_challenges_user (family_id,code,results,created_date,id) VALUES (",apply(results_challenge,1,function(x){paste0(x,collapse = ',')}),");")
  sapply(sql_insert, function(x){dbSendQuery(mydb, x)})

  id.max <- max(results_challenge$id)
  ...
  assign_new_challenges <- function(df){
    ...
    if(NROW(highest_family)==1){ ### /!\ family with id = 9, 453 doesn't appear in [consumption_challenges] table
      ...
      if(current.month %% 3 == 0){ # June, September or Dicember
      ...
      df.return <- data.frame(code = challenge.code,
                              results = 'AVAILABLE',
                              challenge_start_date = floor_date(current_date,unit='day'),
  ...
  new_challenge$id <- id.max+(1:nrow(new_challenge))
...
ans <- paste("AutoUpdated_ConsumptionChallenges_User successfully completed in",round(time_taken[3],2),"seconds.")
print(ans)
