...
PunggolConsumption_SUB <- Punggol_last6months %>%
  ...
  select(service_point_sn,block,room_type,floor,adjusted_consumption,adjusted_date,month,day) %>%
...
DailyPunggolConsumption_last3months <- PunggolConsumption_SUB %>% 
                                       dplyr::mutate(year=year(adjusted_date),date=date(adjusted_date),highest_day=weekdays(adjusted_date)) %>%
...
highest_day_tmp <- DailyPunggolConsumption_last3months 
...
DailyPunggolConsumption_lastmonth <- PunggolConsumption_SUB %>%
                                    dplyr::mutate(year=year(adjusted_date),wd=weekdays(adjusted_date), 
...
consumption_challenges$family_id <- family_servicepoint$id.x[match]
...
ans <- paste("AutoUpdated_ConsumptionChallenges successfully completed in",round(time_taken[3],2),"seconds on",now())
print(ans)
