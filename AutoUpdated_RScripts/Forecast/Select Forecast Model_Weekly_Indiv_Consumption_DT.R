...
X <- X %>% group_by(service_point_sn,Date,week,room_type,block) %>% dplyr::summarise(Consumption = sum(adjusted_consumption))

X$holiday <- (X$Date %in% holidays$Date)*1

Indiv <- X %>% group_by(service_point_sn,week) %>% dplyr::summarise(Consumption=sum(Consumption),Holidays=sum(holiday)) 
