...
Block <- X %>% group_by(block,week) %>% dplyr::summarise(Consumption=sum(Consumption),
...
Efficiency_forecast <- function(my_data,nb.bootstrap = nb.bootstrap){
  ...
  res_final <- cbind(my_data$block[1],res)
  colnames(res_final) <- c('block',model.name)
  return(res_final)
...
EFF <- ddply(Block,.(block),.fun = Efficiency_forecast,nb.bootstrap,.progress = 'text')
...
save(EFF,file = '/srv/shiny-server/DataAnalyticsPortal/data/Select Forecast Model_Weekly_Block_Consumption.RData')
