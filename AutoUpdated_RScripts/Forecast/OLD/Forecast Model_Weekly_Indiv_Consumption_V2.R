...
X1 <- inner_join(X,family_servicepoint,by="service_point_sn") %>%
      dplyr::filter(date(date_consumption) >= move_in_date)  # take into consideration of move-in-date
                                                           
Indiv <- X1 %>% group_by(service_point_sn,week) %>% dplyr::summarise(Consumption=sum(Consumption),
...
# my_data <- Indiv %>% filter(service_point_sn == '3004480211')  # weekly consumption
Forecast <- function(my_data, model = NULL, specify.model = NULL){
  ...
  final.model <- ifelse(is.null(specify.model) | is.na(match(my_data$service_point_sn[1],specify.model$service_point_sn)), 
                        model, specify.model$bestmodel[match(my_data$service_point_sn[1],specify.model$service_point_sn)])
  type.model <- substr(final.model,1,2)
  
  period.ml <- 20  # (previous 19 weeks)
  n <- nrow(my_data)
  ...
  if(type.model=='ML'){
    ...
    ###############  
    period.ts <- 365.25/7
    my_xts <- ts(my_data$Consumption[((n-20):n)],start = my_data$Date[1],frequency = period.ts)
    xreg.train <- my_data$Holidays[((n-20):n)]
  ...
  res <- data.frame(service_point_sn = my_data$service_point_sn[1], week = Week.date$week[match(my_data$week[n],Week.date$week)+1],Model = final.model, Forecast.Consumption = pred)
  return(res)
